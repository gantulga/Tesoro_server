...
class Commodity_category(Createdinfo):
    ...
    division = models.ForeignKey('structure_app.Division', on_delete=models.PROTECT, db_table="product_app_division_commodity_categories", related_name="commodity_categories")
...
class Ingredient(Createdinfo):
    ...
    class Meta:
        verbose_name_plural = "Бүтээгдэхүүн материалын орц"
...
class Item_transfer(Modifiedinfo):
    ...
    # Худалдагдах
    order_detial = models.ForeignKey('payment_app.Order_detial', related_name='item_transfers', null=True, blank=True, on_delete=models.DO_NOTHING)
    fr_division = models.ForeignKey('structure_app.Division', related_name='sending_item_transfers', null=True, blank=True, on_delete=models.DO_NOTHING)
    to_division = models.ForeignKey('structure_app.Division', related_name='coming_item_transfers', null=True, blank=True, on_delete=models.DO_NOTHING)
    fr_client = models.ForeignKey('structure_app.Client', related_name='sending_item_transfers', null=True, blank=True, on_delete=models.DO_NOTHING)
    to_client = models.ForeignKey('structure_app.Client', related_name='coming_item_transfers', null=True, blank=True, on_delete=models.DO_NOTHING)
    fr_user = models.ForeignKey(User, related_name='sending_item_transfers', null=True, blank=True, on_delete=models.DO_NOTHING)
    to_user = models.ForeignKey(User, related_name='coming_item_transfers', null=True, blank=True, on_delete=models.DO_NOTHING)
    # Энэ шилжүүлгийг хянасан эсэх? мөн хэн хянасан
    is_confirmed = models.BooleanField(default=0)
    confirmed_by = models.ForeignKey(User, related_name='item_transfer_confirmed_by', null=True, blank=True, on_delete=models.DO_NOTHING)
    # Худалдан авч буй тохиолдолд доорх утгыг бөглөнө
    store = models.ForeignKey('Store', null=True, blank=True, on_delete=models.PROTECT, related_name="bought_item_transfers")
    recieved_ebarimt = models.BooleanField(default=0)
    comment = models.TextField(null=True, blank=True)
    budget = models.ForeignKey('financial_app.Budget', null=True, blank=True, on_delete=models.DO_NOTHING, related_name="bought_item_transfers")
